...
public class NetSerializer implements Serializer {
  ...
  public Object fromBinary(ByteBuf buf, Optional<Object> hint) {
    ...
    switch (type) {
      ...
      case MSG:
        {
          THeader header = headerFromBinary(buf); // 15-23 bytes
  ...
  private THeader headerFromBinary(ByteBuf buf) {
    TAddress src = addressFromBinary(buf); // 7-11 bytes
    TAddress dst = addressFromBinary(buf); // 7-11 bytes
    int protoOrd = buf.readByte(); // 1 byte
    Transport proto = Transport.values()[protoOrd];
    return new THeader(src, dst, proto); // total of 15-23 bytes, check
  }
  ...
  private void addressToBinary(TAddress addr, ByteBuf buf) {
    ...
    if (addr.getId() != null) {
      buf.writeByte(
          addr.getId()
              .length); // 1 byte - we only want to use 4 bytes for ids, so let's not waste space
      buf.writeBytes(addr.getId()); // should be 4 bytes
      // total of 11 bytes
    } else {
      buf.writeByte(
          -1); //  we'll use this a null-marker (while a length of 0 indicates an empty array)
      // total of 7 bytes
    }
  }
  ...
  private TAddress addressFromBinary(ByteBuf buf) {
    ...
    try {
      ...
      if (idLength >= 0) {
        ...
        buf.readBytes(id);
        return new TAddress(ip, port, id); // total of 11, check
      } else {
